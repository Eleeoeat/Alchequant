...
![Alchequant 能力地图](assets/readme-experience-map.svg)
...
![从本地数据到研究报告](assets/readme-workflow.svg)
...
`notebooks/` 不是另一套应用代码，而是一条可以阅读和复现的研究路线。

代码看起来不多，是因为正式逻辑都放在 `src/` 里。Notebook 负责把这些模块串起来，展示每一步怎么得到结果。这样比在 Notebook 里复制一份复杂代码更稳，也更不容易过期。
